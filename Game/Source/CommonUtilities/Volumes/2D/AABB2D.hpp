...
#pragma region METHOD_DEFINITIONS
	...
	{
		if (myMinPoint.x > aPoint.x) { return false; }
		if (myMinPoint.y > aPoint.y) { return false; }

		if (myMaxPoint.x < aPoint.x) { return false; }
		if (myMaxPoint.y < aPoint.y) { return false; }
	...
	{
		auto difference = myMaxPoint - myMinPoint;
		return myMinPoint + (difference * 0.5f);
	}
